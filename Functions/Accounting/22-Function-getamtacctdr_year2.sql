...
-- Example:
-- SELECT getamtacctdr_year2(
--date_part('YEAR'::text, now()::timestamp) - 0, -- This Year - 0
--'iscogs'  	                                 -- Cost of goods sold
--, 'A')                                         -- Posting type Actual
CREATE OR REPLACE FUNCTION getamtacctdr_year2(p_year double precision, p_acct_category character varying, p_postingtype char)
  RETURNS numeric AS
$BODY$
DECLARE
  v_amtacctdr numeric=0;
...
BEGIN
  ...
  v_sql = 'SELECT SUM(amtacctdr) FILTER (WHERE date_part(''YEAR''::text, dateacctyear)=$1)
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION getamtacctdr_year2(double precision, character varying, char)	-- definition of footprint (how to show function in e.g. PgAdmin tree)
  OWNER TO adempiere;
