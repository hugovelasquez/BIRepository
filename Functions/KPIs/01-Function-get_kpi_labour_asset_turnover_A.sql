...
-- Example:
-- SELECT  
-- get_kpi_labour_asset_turnover_A(1000000, 1000002, date_part('YEAR'::text, now()::timestamp) - 3, 'A') as "Labour Asset Turnover"
CREATE OR REPLACE FUNCTION get_kpi_labour_asset_turnover_A(p_ad_client_id numeric, p_c_element_id numeric, p_year double precision, p_postingtype char)
  RETURNS numeric AS
$BODY$
DECLARE
...
BEGIN
  ...
  v_income_balance = getamtacctbalance(
    p_year,                                                    							-- Year
    getacctidsbystring(p_ad_client_id, p_c_element_id, getacctvaluesincomesales()), 	-- Cuentas Ingresos VENTAS
  ...
  v_discount_balance = getamtacctbalance(
    p_year,                                                    							-- Year
    getacctidsbystring(p_ad_client_id, p_c_element_id, getacctvaluesdiscountsales()), 	-- Cuentas REBAJAS Y DEVOLUCIONES VENTAS ("sobre Compras")
    p_postingtype,                                             							-- Posting Type
    1                                                          							-- Multiplier= x1
  ) ; 
  ...
  v_sales_expense_balance = getamtacctbalance(
    p_year,                                                    							-- Year
    getacctidsbyarray(p_ad_client_id, p_c_element_id, getacctvaluesexpensessales()),    -- Cuentas GASTO VENTAS                                                  -- Cuentas GASTO VENTAS
    p_postingtype,                                             							-- Posting Type
    1                                                         							-- Multiplier= x1
  ); 
  ...
  v_administrative_expense_balance = getamtacctbalance(
    p_year,                                                    							-- Year
    getacctidsbyarray(p_ad_client_id, p_c_element_id, getacctvaluesexpensesadmin()),    -- Cuentas GASTO ADMINISTRACION                                                  							-- Cuentas GASTO ADMINISTRACION 
    p_postingtype,                                            							-- Posting Type
    1                                                         							-- Multiplier= x1
    ) ;                                                                                 
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION get_kpi_labour_asset_turnover_A(numeric, numeric, double precision, char)
  OWNER TO adempiere;
