-- Calculates inventory holding period for one year
-- inventory holding period = average inventory / sales revenue * 365
-- Example:
-- SELECT  
-- get_kpi_inventory_holding_period(1000000, 1000002, date_part('YEAR'::text, now()::timestamp) - 0, 'A') as "Inventory Holding Period"
CREATE OR REPLACE FUNCTION get_kpi_inventory_holding_period(p_ad_client_id numeric, p_c_element_id numeric, p_year double precision, p_postingtype char)
  RETURNS numeric AS
$BODY$
DECLARE
	...
	v_inventory_holding_period numeric;  						-- Return variable = sales revenue / average inventory costs
BEGIN
  v_inventory_holding_period = 0;
  ...
  v_inventory_industrial_balance_curr_year = getamtacctbalance_year(
   ...
   v_inventory_others_balance_pre_year = getamtacctbalance_year(
    ...
    IF (v_average_inventory_costs IS NULL OR v_average_inventory_costs=0 OR v_sales_revenue IS NULL OR v_sales_revenue=0)
    THEN v_inventory_holding_period = 0;
    ELSE v_inventory_holding_period =  v_average_inventory_costs / v_sales_revenue * 365;	-- Result is given in "days"
    END IF;

  RETURN ROUND(COALESCE(v_inventory_holding_period, 0), 5);
END;

$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION get_kpi_inventory_holding_period(numeric, numeric, double precision, char)
  OWNER TO adempiere;
